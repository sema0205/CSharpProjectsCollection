...
public class MoveHandler : ICommandHandler
{
    ...
    private IArgumentHandler<FileMoveBuilder>? _nextArgumentHandler;

    public CommandHandlerResult HandleCommandRequest(CommandHandlerContext commandHandlerContext)
    {
        if (commandHandlerContext.CommandIterator.GetCurrent() != "move")
        {
    ...
    public IArgumentHandler<FileMoveBuilder> SetNextArgumentHandler(IArgumentHandler<FileMoveBuilder> commandHandler)
    {
        if (_nextArgumentHandler is null)
        {
